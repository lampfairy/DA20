...
function scene:create( event )
	...
	local object = {}
 	...
 	object[2].x, object[2].y = display.contentWidth * 0.75 - 50, display.contentHeight * 0.7 - 20
 	object[3] = display.newImage("image/cursor_1.png")
 	object[3].x, object[3].y = 450, 1050
 	object[4] = display.newImage("image/cursor_2.png")
 	object[4].x, object[4].y = display.contentWidth * 0.5, display.contentHeight * 0.4
	...
	local function cursorMove(event)
    	...
    	transition.moveTo( object[3], { x=600, y=1080, time=1000 } )
	end

 	local function nextScene( event )
 		print("opening2 로 이동합니다.")
 		composer.gotoScene( "opening2_1" )
 	end
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		composer.removeScene('opening1_2')
